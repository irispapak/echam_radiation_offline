...
module mo_lrtm_taumol
  ...
  use parrrtm, only : mg, nbndlw, maxxsec, ngptlw, nspa, nspb
...
contains
  ...
  subroutine lrtm_taumol(kproma, nlayers, pavel, wx, coldry, &
    ...
    real(wp) :: zlaytrop(kproma,nlayers)
    integer :: icl, ich, jl, lay
  ...
  contains
    ...
    subroutine taugb3
      ...
      ! Local 
      integer(widx) :: lay, ig, ixc0, ixp
      integer(widx) :: ind0, ind1, inds, indf, indm
      integer(widx) :: js(kproma), js1(kproma), jmn2o(kproma), jpl(kproma)
      real(wp) :: speccomb(kproma),specparm(kproma),fs(kproma),specmult
      real(wp) :: speccomb1(kproma),specparm1(kproma), fs1(kproma),specmult1
      real(wp) :: speccomb_mn2o, specparm_mn2o, specmult_mn2o
      real(wp) :: fmn2o(kproma), chi_n2o, ratn2o, adjfac, adjcoln2o(kproma)
! not used, see below     real(wp) :: fmn2omf
      real(wp) :: speccomb_planck, specparm_planck, specmult_planck, fpl(kproma)
      ...
      real(wp) :: refrat_planck,refrat_m,ztabsize
      ...
      do lay = 1, nlayers
         ...
         do jl = 1, kproma
            ...
            refrat_m      = FSEL(zlaytrop(jl,lay),refrat_m_a     ,refrat_m_b     )
            refrat_planck = FSEL(zlaytrop(jl,lay),refrat_planck_a,refrat_planck_b)
            ztabsize      = FSEL(zlaytrop(jl,lay),8._wp          ,4._wp          )
          
            speccomb(jl)    = colh2o(jl,lay) + rat_h2oco2(jl,lay)  *colco2(jl,lay)
            speccomb1(jl)   = colh2o(jl,lay) + rat_h2oco2_1(jl,lay)*colco2(jl,lay)
            speccomb_mn2o   = colh2o(jl,lay) + refrat_m            *colco2(jl,lay)
            speccomb_planck = colh2o(jl,lay) + refrat_planck       *colco2(jl,lay)
            
            specparm(jl)    = MIN(colh2o(jl,lay)/speccomb(jl),oneminus)
            specparm1(jl)   = MIN(colh2o(jl,lay)/speccomb1(jl),oneminus)
            specparm_mn2o   = MIN(colh2o(jl,lay)/speccomb_mn2o,oneminus)
            specparm_planck = MIN(colh2o(jl,lay)/speccomb_planck,oneminus)
            
            specmult        = ztabsize*specparm(jl)
            specmult1       = ztabsize*specparm1(jl)
            specmult_mn2o   = ztabsize*specparm_mn2o
            specmult_planck = ztabsize*specparm_planck
          
            fs(jl)    = specmult        - AINT(specmult)
            fs1(jl)   = specmult1       - AINT(specmult1)
            fmn2o(jl) = specmult_mn2o   - AINT(specmult_mn2o)
            fpl(jl)   = specmult_planck - aint(specmult_planck)

            ! mpuetz: this is never used ?
            ! fmn2omf = minorfrac(jl,lay)*fmn2o

            js(jl)    = INT(specmult + 1.0_wp)
            js1(jl)   = INT(specmult1 + 1.0_wp)
            jmn2o(jl) = INT(specmult_mn2o + 1.0_wp)
            jpl(jl)   = INT(specmult_planck + 1.0_wp)
         end do
         ...
         ! mpuetz: separated this loop from the rest, so it doesn't interfere
         ! with pipelining the other loops

         do jl = 1, kproma
            chi_n2o = coln2o(jl,lay)/coldry(jl,lay)
            ratn2o = 1.e20_wp*chi_n2o/chi_mls(4,jp(jl,lay)+1) - 0.5_wp
            adjcoln2o(jl) = coln2o(jl,lay)
            if (ratn2o .gt. 1.0_wp) then
               adjfac = 0.5_wp + ratn2o**0.65_wp
               adjcoln2o(jl) = adjfac*chi_mls(4,jp(jl,lay)+1)*coldry(jl,lay)*1.e-20_wp
            endif
         end do
         ...
         do ixp = 1, ixc0
            ...
            ! mpuetz: this is still ugly, but can't help it with a non-uniform interpolation scheme
            ! it would be better to use cubic splines all over and prehaps increase the resolution
            ! of the tables a bit
            
            if (specparm(jl) .lt. 0.125_wp) then
               p = fs(jl) - 1.0_wp
               p4 = p**4
               fk0 = p4
               fk1 = 1.0_wp - p - 2.0_wp*p4
               fk2 = p + p4
            ...
            else if (specparm(jl) .gt. 0.875_wp) then
               p = -fs(jl) 
               p4 = p**4
               fk0 = p4
               fk1 = 1.0_wp - p - 2.0_wp*p4
               ...
               fac000 = (1._wp - fs(jl)) * fac00(jl,lay)
               fac010 = (1._wp - fs(jl)) * fac10(jl,lay)
               fac100 = fs(jl) * fac00(jl,lay)
               fac110 = fs(jl) * fac10(jl,lay)
            endif
          
            if (specparm1(jl) .lt. 0.125_wp) then
               p = fs1(jl) - 1.0_wp
               p4 = p**4
               fk0 = p4
               fk1 = 1 - p - 2.0_wp*p4
            ...
            else if (specparm1(jl) .gt. 0.875_wp) then
               p = -fs1(jl)
               p4 = p**4
               fk0 = p4
               fk1 = 1 - p - 2.0_wp*p4
               ...
               fac001 = (1._wp - fs1(jl)) * fac01(jl,lay)
               fac011 = (1._wp - fs1(jl)) * fac11(jl,lay)
               fac101 = fs1(jl) * fac01(jl,lay)
               fac111 = fs1(jl) * fac11(jl,lay)
            endif

            ! mpuetz: pulled the interpolation scheme branches out of the ig loops
            ! so the ig loops either get fully unrolled or pipelined
            ! (let the compiler decide based on its own heuristics)
          
            ind0 = ((jp(jl,lay)-1)*5+(jt(jl,lay)-1))*nspa(3) + js(jl)
            
            if (specparm(jl) .lt. 0.125_wp) then
               tau_major(1:ng3) = speccomb(jl) * &
            ...
            ind1 = (jp(jl,lay)*5+(jt1(jl,lay)-1))*nspa(3) + js1(jl)

            if (specparm1(jl) .lt. 0.125_wp) then
               tau_major1(1:ng3) = speccomb1(jl) * &
            ...
            do ig = 1, ng3
               ...
               taufor = forfac(jl,lay) * (forref(indf,ig) + forfrac(jl,lay) * &
                    (forref(indf+1,ig) - forref(indf,ig))) 
               n2om1 = ka_mn2o(jmn2o(jl),indm,ig) + fmn2o(jl) * &
                    (ka_mn2o(jmn2o(jl)+1,indm,ig) - ka_mn2o(jmn2o(jl),indm,ig))
               n2om2 = ka_mn2o(jmn2o(jl),indm+1,ig) + fmn2o(jl) * &
                    (ka_mn2o(jmn2o(jl)+1,indm+1,ig) - ka_mn2o(jmn2o(jl),indm+1,ig))
               absn2o = n2om1 + minorfrac(jl,lay) * (n2om2 - n2om1)
               
               taug(jl,lay,ngs2+ig) = tau_major(ig) + tau_major1(ig) &
                    + tauself + taufor &
                    + adjcoln2o(jl)*absn2o
               fracs(jl,lay,ngs2+ig) = fracrefa(ig,jpl(jl)) + fpl(jl) * &
                    (fracrefa(ig,jpl(jl)+1)-fracrefa(ig,jpl(jl)))
            enddo
         enddo
         ...
         ! mpuetz: this loop is much nicer for the compiler & hardware
         ! because the interpolation scheme is uniform -> no branches
         ...
         do ixp = 1, ixc0
            ...
            ind0 = ((jp(jl,lay)-13)*5+(jt(jl,lay)-1))*nspb(3) + js(jl)

            tau_major(1:ng3) = speccomb(jl) * &
                 (fac000 * absb(ind0,1:ng3) + &
                 fac100 * absb(ind0+1,1:ng3) + &
                 fac010 * absb(ind0+5,1:ng3) + &
                 fac110 * absb(ind0+6,1:ng3))

            ind1 = ((jp(jl,lay)-12)*5+(jt1(jl,lay)-1))*nspb(3) + js1(jl)
            
            tau_major1(1:ng3) = speccomb1(jl) * &
                 (fac001 * absb(ind1,1:ng3) +  &
                 fac101 * absb(ind1+1,1:ng3) + &
                 fac011 * absb(ind1+5,1:ng3) + &
                 fac111 * absb(ind1+6,1:ng3))
            ...
            do ig = 1, ng3
               taufor = forfac(jl,lay) * (forref(indf,ig) + &
                    forfrac(jl,lay) * (forref(indf+1,ig) - forref(indf,ig))) 
               n2om1 = kb_mn2o(jmn2o(jl),indm,ig) + fmn2o(jl) * &
                    (kb_mn2o(jmn2o(jl)+1,indm,ig)-kb_mn2o(jmn2o(jl),indm,ig))
               n2om2 = kb_mn2o(jmn2o(jl),indm+1,ig) + fmn2o(jl) * &
                    (kb_mn2o(jmn2o(jl)+1,indm+1,ig)-kb_mn2o(jmn2o(jl),indm+1,ig))
               absn2o = n2om1 + minorfrac(jl,lay) * (n2om2 - n2om1)
               taug(jl,lay,ngs2+ig) = tau_major(ig) + tau_major1(ig) &
                    + taufor &
                    + adjcoln2o(jl)*absn2o
               fracs(jl,lay,ngs2+ig) = fracrefb(ig,jpl(jl)) + fpl(jl) * &
                    (fracrefb(ig,jpl(jl)+1)-fracrefb(ig,jpl(jl)))
            enddo
         enddo
    ...
    subroutine taugb4
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          specparm = colh2o(jl,lay)/speccomb
          ...
          specparm1 = colh2o(jl,lay)/speccomb1
          ...
          specparm_planck = colh2o(jl,lay)/speccomb_planck
          ...
          do ig = 1, ng4
            ...
            taug(jl,lay,ngs3+ig) = tau_major + tau_major1 &
                 + tauself + taufor
        ...
        do ixp = 1, ixc0
          ...
          specparm = colo3(jl,lay)/speccomb
          if (specparm .ge. oneminus) specparm = oneminus
          ...
          specparm1 = colo3(jl,lay)/speccomb1
          if (specparm1 .ge. oneminus) specparm1 = oneminus
          ...
          specparm_planck = colo3(jl,lay)/speccomb_planck
          if (specparm_planck .ge. oneminus) specparm_planck=oneminus
        ...
        do ixp = 1, ixc0
          jl = ixhigh(ixp,lay)
          taug(jl,lay,ngs3+8)=taug(jl,lay,ngs3+8)*0.92
          taug(jl,lay,ngs3+9)=taug(jl,lay,ngs3+9)*0.88
          taug(jl,lay,ngs3+10)=taug(jl,lay,ngs3+10)*1.07
          taug(jl,lay,ngs3+11)=taug(jl,lay,ngs3+11)*1.1
          taug(jl,lay,ngs3+12)=taug(jl,lay,ngs3+12)*0.99
          taug(jl,lay,ngs3+13)=taug(jl,lay,ngs3+13)*0.88
          taug(jl,lay,ngs3+14)=taug(jl,lay,ngs3+14)*0.943
        enddo
    ...
    subroutine taugb5
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          specparm_mo3 = colh2o(jl,lay)/speccomb_mo3
          ...
          do ig = 1, ng5
            ...
            taug(jl,lay,ngs4+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + abso3*colo3(jl,lay) &
                 + wx(jl,1,lay) * ccl4(ig)
    ...
    subroutine taugb7
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          specparm_mco2 = colh2o(jl,lay)/speccomb_mco2
          ...
          !  to obtain the proper contribution.
          chi_co2 = colco2(jl,lay)/(coldry(jl,lay))
          ratco2 = 1.e20*chi_co2/chi_mls(2,jp(jl,lay)+1)
          if (ratco2 .gt. 3.0_wp) then
          ...
          do ig = 1, ng7
            ...
            taug(jl,lay,ngs6+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + adjcolco2*absco2
    ...
    subroutine taugb9
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          specparm_mn2o = colh2o(jl,lay)/speccomb_mn2o
          ...
          do ig = 1, ng9
            ...
            taug(jl,lay,ngs8+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + adjcoln2o*absn2o
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          do ig = 1, ng12
            ...
            taug(jl,lay,ngs11+ig) = tau_major + tau_major1 &
                 + tauself + taufor
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          if (ratco2 .gt. 3.0_wp) then
            ...
            adjcolco2 = adjfac*3.55e-4*coldry(jl,lay)*1.e-20_wp
          else
            adjcolco2 = colco2(jl,lay)
          endif

          speccomb_mco = colh2o(jl,lay) + refrat_m_a3*coln2o(jl,lay)
          specparm_mco = colh2o(jl,lay)/speccomb_mco
          ...
          do ig = 1, ng13
            ...
            taug(jl,lay,ngs12+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + adjcolco2*absco2 &
                 + colco(jl,lay)*absco
    ...
    subroutine taugb15
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          specparm = coln2o(jl,lay)/speccomb
          if (specparm .ge. oneminus) specparm = oneminus
          specmult = 8._wp*(specparm)
          js = 1 + int(specmult)
          fs = mod(specmult,1.0_wp)
          ...
          specparm1 = coln2o(jl,lay)/speccomb1
          if (specparm1 .ge. oneminus) specparm1 = oneminus
          specmult1 = 8._wp*(specparm1)
          js1 = 1 + int(specmult1)
          fs1 = mod(specmult1,1.0_wp)
          ...
          specparm_mn2 = coln2o(jl,lay)/speccomb_mn2
          ...
          specparm_planck = coln2o(jl,lay)/speccomb_planck
          if (specparm_planck .ge. oneminus) specparm_planck=oneminus
          specmult_planck = 8._wp*specparm_planck
          jpl = 1 + int(specmult_planck)
          fpl = mod(specmult_planck,1.0_wp)
          ...
          do ig = 1, ng15
            ...
            taug(jl,lay,ngs14+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + taun2
      ...
      do lay = 1, nlayers
        ...
        do ixp = 1, ixc0
          ...
          do ig = 1, ng16
            ...
            taug(jl,lay,ngs15+ig) = tau_major + tau_major1 &
                 + tauself + taufor
