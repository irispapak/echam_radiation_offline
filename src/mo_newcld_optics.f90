...
!  USE mo_exception, ONLY: finish
  ...
  REAL (wp), SAVE :: &
       ...
       zinhoml2        !< value for liquid-cloud inhomogeneity factor with convection  
...
!    CALL p_bcast(z_asy_i   , p_io)
  ...
  SUBROUTINE newcld_optics(                                               &
    ...
    IF (relmin < 1.5 .OR. relmin > 2.5 ) THEN
      write(*,*) 'Apparently unsuccessful loading of optical tables'
    END IF
    !
    ...
    ! --------------------------------
    zfact = 1.0e6_wp*(3.0e-9_wp/(4.0_wp*api*rhoh2o))**(1.0_wp/3.0_wp)
    DO jk=1,klev
      DO jl=1,kproma
        ...
        END IF
        tau_lw(jl,jk,1:nb_lw) = ztau(jl,jk,1:16) * (1.0_wp - zomg(jl,jk,1:16))
        tau_sw(jl,1:nb_sw,jk) = ztau(jl,jk,16:29)
        omg(jl,1:nb_sw,jk)    = zomg(jl,jk,16:29)
        asy(jl,1:nb_sw,jk)    = zasy(jl,jk,16:29)
      END DO
    END DO
