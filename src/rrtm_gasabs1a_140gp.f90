...
INTEGER :: IXC(KLEV), IXLOW(KBDIM,KLEV), IXHIGH(KBDIM,KLEV), ITRV(KBDIM)
...
!#ifdef _PROFILE
!  CALL trace_start ('rrtm_gasabs1a_140gp', 4)
!#endif
...
DO LAY = 1, KLEV
  DO IPR = 1, JPGPT
    ...
    DO IPLON = 1, KPROMA
      ...
      TF = SWDIV_NOCHK(ODEPTH,(BPADE+ODEPTH))
      TF = FSEL(-ODEPTH,0._DP,TF)
      IR=(LAY-1)*JPGPT+IPR
      TAUSF1(IPLON,IR) = TF
      ITRV(IPLON)=INT(5.E+03_DP*TF+0.5_DP)
    ENDDO
    IF (KPROMA >= 1) THEN
       DO IPLON = 1, KPROMA
          ABSS1 (IPLON,IR) = 1._DP - TRANS(ITRV(IPLON))
       ENDDO
    END IF
  ENDDO
ENDDO
!
!#ifdef _PROFILE
!  CALL trace_stop ('rrtm_gasabs1a_140gp', 4)
!#endif
!
