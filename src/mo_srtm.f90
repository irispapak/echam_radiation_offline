...
MODULE mo_srtm
  ...
  USE mo_constants, ONLY : g, rd, NDAYLEN=>DAYLEN ! LT
!LT
!  USE mo_time_control, ONLY : ndaylen
  USE mo_srtm_config, ONLY : delwave, wavenum2, wavenum1, &
       &    ngc, jpinpx, jpb1, jpb2, jpgpt, preflog, tref, repclc, replog, &
       &    ssi_default
...
CONTAINS
  ...
  SUBROUTINE srtm_srtm_224gp &
       ...
       &  pm_fl_vr        ,tk_fl_vr        ,pp_thick_fl     ,prmu0           ,&
       &  col_dry_vr      ,wkl_vr          ,cld_frc_vr      ,cld_tau_sw_vr   ,&
       &  cld_cg_sw_vr    ,cld_piz_sw_vr   ,aer_tau_sw_vr   ,aer_cg_sw_vr    ,&
       &  aer_piz_sw_vr   ,psctm           ,ssi_factor      ,pfuvf           ,&
       &  htngrt          ,flxd_sw         ,flxu_sw         ,htngrt_clr      ,&
       &  flxd_sw_clr     ,flxu_sw_clr     ,flxd_nir_all_sfc,flxd_vis_all_sfc,&
       &  flxu_nir_all_toa,flxu_vis_all_toa,flxd_nir_clr_sfc,flxd_vis_clr_sfc,&
       &  flxu_nir_clr_toa,flxu_vis_clr_toa,psudu           ,flxn_nir_all_sfc,&
       &  dff_frc_nir_sfc ,flxn_vis_all_sfc,dff_frc_vis_sfc ,flxd_par_all_sfc,&
       &  dff_frc_par_sfc)
    ...
    REAL(wp), INTENT(in)    :: pp_thick_fl(kbdim,klev) 
...
!   REAL(wp), INTENT(in)    :: aer_piz_sw_vr(kbdim,ksw,klev)
    ...
    REAL(wp), INTENT(in)    :: psctm !< solar constant for radiation time step
    REAL(wp), INTENT(in)    :: ssi_factor(ksw)
    REAL(wp), INTENT(out)   :: psudu(kbdim), pfuvf(kbdim)
    ...
    REAL(wp)   ,INTENT(out)   ::       &
         ...
         flxd_nir_clr_sfc(kbdim),      & !< surf. NIR downw.
         flxd_vis_clr_sfc(kbdim),      & !< surf. vis. downw.
         flxd_nir_all_sfc(kbdim),      & !< surf. NIR downw. cloudy sky
         flxd_par_all_sfc(kbdim),      & !< surf. PAR downw. total sky
         flxd_vis_all_sfc(kbdim),      & !< surf. vis. downw. cloudy sky
         flxu_nir_clr_toa(kbdim),      & !< top NIR upw.
         flxu_vis_clr_toa(kbdim),      & !< top vis. upw.
         flxu_nir_all_toa(kbdim),      & !< top NIR upw. cloudy sky
         flxu_vis_all_toa(kbdim),      & !< top vis. upw. cloudy sky
         flxn_nir_all_sfc(kbdim),      & !< net surface NIR flux
         flxn_vis_all_sfc(kbdim),      & !< net surface visible flux
         dff_frc_nir_sfc(kbdim),       & !< fraction of diffuse NIR
         dff_frc_vis_sfc(kbdim),       & !< fraction of diffuse visible
         dff_frc_par_sfc(kbdim),       & !< fraction of diffuse PAR at the
                                         !   surface
    ...
    REAL(wp) :: zbbfu(kbdim,klev+1,ksw)
    REAL(wp) :: zsudu(kbdim,ksw), zsuduc(kbdim,ksw), zbbfd(kbdim,klev+1,ksw)
    ...
    REAL(wp) :: zclear(kbdim), zcloud(kbdim), ztotcc(kbdim)
    REAL(wp) :: zalbd(kbdim,ksw) , zalbp(kbdim,ksw), zrmu0(kbdim)
    REAL(wp) :: frc_vis(ksw), frc_nir(ksw)
    REAL(wp) :: zeps, rcday, frc_par, zfvis, zfnir, zfpar, total
    INTEGER(widx) :: idx(kbdim)
    INTEGER(widx) :: icount, ic, jl, jk, jsw, jb

    !-----------------------------------------------------------------------
    !-- calculate information needed ny the radiative transfer routine 
!LT
!#ifdef _PROFILE
!    CALL trace_start('srtm_srtm_224gp',4)
!#endif

    rcday      = REAL(ndaylen,wp)*g/3.5_wp/rd
    zeps       = 1.e-06_wp
    z_oneminus = 1.0_wp -  zeps
    !

!hs bnd_wght(:) = psctm/1368.22234151 * scycle_factor(:)
...
!IBM* ASSERT(NODEPS)
          DO ic = 1, icount
             ...
             zclear(ic)=zclear(ic)*(1.0_wp-MAX(cld_frc_vr(jl,jk),zcloud(ic))) &
                  & /(1.0_wp-MIN(zcloud(ic),1.0_wp-zeps))  
             zcloud(ic)=cld_frc_vr(jl,jk)
...
!IBM* ASSERT(NODEPS)
          DO ic = 1, icount
             jl = idx(ic)
             zfrcl(ic,jk) = cld_frc_vr(jl,jk)
             zcloud(ic)=MAX(zcloud(ic),cld_frc_vr(jl,jk))
...
!IBM* ASSERT(NODEPS)
          DO ic = 1, icount
             jl = idx(ic)
             zfrcl(ic,jk) = cld_frc_vr(jl,jk)
             zclear(ic)=zclear(ic)*(1.0_wp-cld_frc_vr(jl,jk))
    ...
    CALL srtm_setcoef &
         & ( icount   , kbdim    , klev   , idx, &
         & pm_fl_vr   , tk_fl_vr , &
         & col_dry_vr , wkl_vr    , i_laytrop    , i_layswtch   , i_laylow  ,&
         & z_co2mult  , z_colch4  , z_colco2     , z_colh2o     , z_colmol  ,&
         & z_coln2o   , z_colo2   , z_colo3      , z_forfac     , z_forfrac ,&
         & indfor     , z_selffac , z_selffrac   , indself      , z_fac00   ,&
         & z_fac01    , z_fac10   , z_fac11      , jp           , jt        ,&
         & jt1        , zrmu0   )  
...
!IBM* ASSERT(NODEPS)
   ...
   ENDDO
    ...
    CALL srtm_spcvrt &
         ...
         & z_oneminus , &
         & zalbd       , zalbp       , zfrcl      , ztauc      , zasyc      , &
         & zomgc       , ztaua       , zasya      , zomga      , zrmu0      , &
         & i_laytrop   , &
         & z_colch4    , z_colco2   , z_colh2o   , z_colmol   ,  &
         & z_colo2     , z_colo3    , z_forfac   , z_forfrac  ,  &
         & indfor      , z_selffac   , z_selffrac , indself    , z_fac00    , &
         & z_fac01     , z_fac10     , z_fac11    , jp         , jt         , &
         & jt1         , zbbfd       , zbbfu      , zbbcd      , zbbcu      , &
         & zsudu       , zsuduc      )  
    ...
    DO jsw=1,ksw
       DO jk=1,klev+1
          DO ic=1,icount
             jl = idx(ic)
             PRINT '(A10,3I4,4E24.15)','srtm_vrt ',jsw,jl,jk,zbbfd(ic,jk,jsw),zbbfu(ic,jk,jsw),zbbcd(ic,jk,jsw),zbbcu(ic,jk,jsw)
          END DO
       END DO
    END DO
#endif
    ! mpuetz: careful here result arrays must be complete
...
!IBM* ASSERT(NODEPS)
         DO ic = 1, icount
            jl = idx(ic)
            flxu_sw(jl,jk)     = flxu_sw(jl,jk) + bnd_wght(jb)*(     &
                 &   zcloud(ic)*zbbfu(ic,jk,jb)                      &
                 & + zclear(ic)*zbbcu(ic,jk,jb))
            flxd_sw(jl,jk)     = flxd_sw(jl,jk) + bnd_wght(jb)*(     &
                 &   zcloud(ic)*zbbfd(ic,jk,jb)                      &
                 & + zclear(ic)*zbbcd(ic,jk,jb)) 
           flxu_sw_clr(jl,jk)=flxu_sw_clr(jl,jk)+bnd_wght(jb)*zbbcu(ic,jk,jb)
           flxd_sw_clr(jl,jk)=flxd_sw_clr(jl,jk)+bnd_wght(jb)*zbbcd(ic,jk,jb)
        ENDDO
     ENDDO
!LT
!     IF (ksw /= 14 ) CALL finish ('srtm_srtm_224gp','Number of bands changed. Please check diagnosis of PAR')
     IF (jb == 9) THEN
!        IF (ABS(wavenum1(jb+jpb1-1)-12850.0_wp) > 1.0_wp .OR. ABS(wavenum2(jb+jpb1-1)-16000.0_wp) > 1.0_wp)  &
!             CALL finish ('srtm_srtm_224gp','Solar band changed. Please check diagnosis of PAR')
        frc_par = 0.533725
     ELSE IF (jb == 10) THEN
!        IF (ABS(wavenum1(jb+jpb1-1)-16000.0_wp) > 1.0_wp .OR. ABS(wavenum2(jb+jpb1-1)-22650.0_wp) > 1.0_wp) &
!             CALL finish ('srtm_srtm_224gp','Solar band changed. Please check diagnosis of PAR')
        frc_par = 1.0_wp    
     ELSE IF (jb == 11) THEN
!        IF (ABS(wavenum1(jb+jpb1-1)-22650.0_wp) > 1.0_wp .OR. ABS(wavenum2(jb+jpb1-1)-29000.0_wp) > 1.0_wp) &
!             CALL finish ('srtm_srtm_224gp','Solar band changed. Please check diagnosis of PAR')
        frc_par = 0.550164
     ELSE
        frc_par = 0.0_wp
...
!IBM* ASSERT(NODEPS)
     DO ic = 1, icount
        jl = idx(ic)
        flxd_nir_clr_sfc(jl) = flxd_nir_clr_sfc(jl)+zfnir*zbbcd(ic,klev+1,jb)
        flxd_vis_clr_sfc(jl) = flxd_vis_clr_sfc(jl)+zfvis*zbbcd(ic,klev+1,jb) 
        ...
        flxu_nir_clr_toa(jl)  = flxu_nir_clr_toa(jl) + zfnir*zbbcu(ic,1,jb) 
        flxu_vis_clr_toa(jl)  = flxu_vis_clr_toa(jl) + zfvis*zbbcu(ic,1,jb)
        flxu_nir_all_toa(jl)  = flxu_nir_all_toa(jl) + zfnir*( &
             &   zcloud(ic)*zbbfu(ic,1,jb)          &
             & + zclear(ic)*zbbcu(ic,1,jb)          )
...
!IBM* NOVECTOR
     DO jl=1,kproma
        htngrt(jl,jk)     = rcday/pp_thick_fl(jl,jk) * (              &
             &                flxu_sw(jl,jk)-flxu_sw(jl,jk+1)         &
             &              - flxd_sw(jl,jk)+flxd_sw(jl,jk+1)         )
        htngrt_clr(jl,jk) = rcday/pp_thick_fl(jl,jk) * (              &
             &                flxu_sw_clr(jl,jk)-flxu_sw_clr(jl,jk+1) &
             &              - flxd_sw_clr(jl,jk)+flxd_sw_clr(jl,jk+1) )
     END DO
  END DO

#ifdef _PROFILE
  CALL trace_stop('srtm_srtm_224gp',4)
#endif

#ifdef CHK_FLX
  DO jk=1,klev
     DO jl=1,kproma
        PRINT '(A10,2I4,4E24.15)','srtm_flx ',jl,jk,flxu_sw(jl,jk),flxd_sw(jl,jk),flxu_sw_clr(jl,jk),flxd_sw_clr(jl,jk)
     END DO
  END DO
#endif
  ...
  SUBROUTINE srtm_setcoef &
    ...
    INTEGER(widx),INTENT(in)    :: icount
    INTEGER,INTENT(in)    :: kbdim
    INTEGER,INTENT(in)    :: klev 
    INTEGER(widx),INTENT(in) :: idx(kbdim)
    ...
    REAL(wp) :: rkjt,rkjt1,rkindfor,rkindself
    ...
    DO jk = 1, i_nlayers
      DO ic = 1,icount
         jl = idx(ic)
         z_plog(ic) = LOG(pavel(jl,jk))
         z_ptaveli(ic) = 1.0_wp/ptavel(jl,jk)
      ...
      DO ic = 1,icount
         ...
         kjp(ic,jk) = INT(MIN(MAX(AINT(36.0_wp - 5.0_wp*(z_plog(ic)+0.04_wp)),1.0_wp),58.0_wp))
         jp1 = kjp(ic,jk) + 1
         z_fp = 5.0_wp * (preflog(kjp(ic,jk)) - z_plog(ic))
         ...
         rkjt = MIN(MAX(AINT(3.0_wp + (ptavel(jl,jk)-tref(kjp(ic,jk)))/15.0_wp),1.0_wp),4.0_wp)
         kjt(ic,jk) = INT(rkjt)

          z_ft = ((ptavel(jl,jk)-tref(kjp(ic,jk)))/15.0_wp) - rkjt + 3.0_wp
          rkjt1 = MIN(MAX(AINT(3.0_wp + (ptavel(jl,jk)-tref(jp1))/15.0_wp),1.0_wp),4.0_wp)
          kjt1(ic,jk) = INT(rkjt1)

          z_ft1 = ((ptavel(jl,jk)-tref(jp1))/15.0_wp) - rkjt1 + 3.0_wp

          z_water = pwkl(jl,1,jk)/pcoldry(jl,jk)
          z_scalefac = pavel(jl,jk) * z_stpfac *z_ptaveli(ic)

          !        If the pressure is less than ~100mb, perform a different
          !        set of species interpolations.

          IF (z_plog(ic) <= 4.56) go to 5300
          klaytrop(ic) =  klaytrop(ic) + 1
          IF (z_plog(ic) >= 6.62) klaylow(ic) = klaylow(ic) + 1

          !        Set up factors needed to separately include the water vapor
          !        foreign-continuum in the calculation of absorption coefficient.

          pforfac(ic,jk) = z_scalefac / (1.+z_water)
          z_factor = (332.0_wp-ptavel(jl,jk))/36.0_wp
          ...
          z_factor = (ptavel(jl,jk)-188.0_wp)/7.2_wp
          ...
          pcolh2o(ic,jk) = 1.e-20_wp * pwkl(jl,1,jk)
          pcolco2(ic,jk) = 1.e-20_wp * pwkl(jl,2,jk)
          pcolo3(ic,jk) = 1.e-20_wp * pwkl(jl,3,jk)
          !         COLO3(LAY) = 0.
          !         COLO3(LAY) = colo3(lay)/1.16
          pcoln2o(ic,jk) = 1.e-20_wp * pwkl(jl,4,jk)
          pcolch4(ic,jk) = 1.e-20_wp * pwkl(jl,6,jk)
          pcolo2(ic,jk) = 1.e-20_wp * pwkl(jl,7,jk)
          pcolmol(ic,jk) = 1.e-20_wp * pcoldry(jl,jk) + pcolh2o(ic,jk)
          !         colco2(lay) = 0.
          !         colo3(lay) = 0.
          !         coln2o(lay) = 0.
          !         colch4(lay) = 0.
          !         colo2(lay) = 0.
          !         colmol(lay) = 0.
          IF (pcolco2(ic,jk) == 0._wp) pcolco2(ic,jk) = 1.e-32_wp * pcoldry(jl,jk)
          IF (pcoln2o(ic,jk) == 0._wp) pcoln2o(ic,jk) = 1.e-32_wp * pcoldry(jl,jk)
          IF (pcolch4(ic,jk) == 0._wp) pcolch4(ic,jk) = 1.e-32_wp * pcoldry(jl,jk)
          IF (pcolo2(ic,jk)  == 0._wp) pcolo2(ic,jk)  = 1.e-32_wp * pcoldry(jl,jk)
          !        Using E = 1334.2 cm-1.
          z_co2reg = 3.55e-24_wp * pcoldry(jl,jk)
          pco2mult(ic,jk)= (pcolco2(ic,jk) - z_co2reg) * 272.63_wp * z_exptavel(ic)
          go to 5400
...
5300      CONTINUE
          ...
          pforfac(ic,jk) = z_scalefac / (1.+z_water)
          z_factor = (ptavel(jl,jk)-188.0_wp)/36.0_wp
          kindfor(ic,jk) = 3
...
5400      CONTINUE
          ...
          pfac00(ic,jk) = z_compfp * (1. - z_ft)
          pfac11(ic,jk) = z_fp * z_ft1
          pfac01(ic,jk) = z_fp * (1. - z_ft1)
  ...
  SUBROUTINE srtm_spcvrt &
       & ( icount  ,  kbdim   , klev      , ksw      , &
       &   poneminus, &
       &   palbd   ,  palbp   , zfrcl     , ptauc    , pasyc    , &
       &   pomgc   ,  ptaua   , pasya     , pomga    , zrmu0    , &
       &   klaytrop, &
       &   pcolch4 , pcolco2  , pcolh2o   , pcolmol  , &
       &   pcolo2  , pcolo3   , pforfac   , pforfrac , &
       &   kindfor , pselffac , pselffrac , kindself , &
       &   pfac00  , pfac01   , pfac10    , pfac11   , &
       &   kjp     , kjt      , kjt1      ,&
                                !-- output arrays 
       &   pbbfd   , pbbfu    , pbbcd   , pbbcu  , psudu , psuduc )

   USE mo_srtm_config, ONLY: ng16,ng17,ng18,ng19,ng20,ng21,ng22,ng23,ng24,ng25,ng26,ng27,ng28,ng29
    ...
    INTEGER,INTENT(in)    :: kbdim
    INTEGER,INTENT(in)    :: klev
    REAL(wp)                  :: poneminus(kbdim) ! UNDETERMINED INTENT
    ...
    REAL(wp)   ,INTENT(inout) :: psudu(kbdim,jpb1:jpb2), psuduc(kbdim,jpb1:jpb2)
    !     ------------------------------------------------------------------
    ...
    REAL(wp) :: &
         & zdbtc(kbdim,klev+1), ztdbtc(kbdim,klev+1), zincflx(kbdim) & ! ,jpgpt)  &
         & ,  zincf14(kbdim,14)   , zinctot(kbdim) 

    INTEGER(widx) :: ib1, ib2, ibm, igt, ikl, jb, jg, jk, ic, ng
    INTEGER :: i_kmodts
    REAL(wp) :: zdbtmc, zdbtmo, zf, zincflux(kbdim), zrmu0i(kbdim), zwf
...
!    REAL(wp) :: &
!         & zcd(kbdim,klev+1,jpgpt), zcu(kbdim,klev+1,jpgpt) &
!         & ,  zfd(kbdim,klev+1,jpgpt), zfu(kbdim,klev+1,jpgpt)  
    REAL(wp) :: &
...
!IBM* NOUNROLL
          DO jk=1,klev
             ...
             !-- and total sky optical parameters        
...
!IBM* NOVECTOR
  ...
  SUBROUTINE srtm_reftra &
    ...
    !     ------------------------------------------------------------------
    INTEGER(widx) :: idxt(kbdim), idxf(kbdim), idxc(kbdim), idxn(kbdim)
    INTEGER(widx) :: jk, ic, jc, ict, icf, icc, icn
