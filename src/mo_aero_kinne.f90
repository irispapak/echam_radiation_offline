...
! !LOCAL VARIABLES
  ...
  REAL(dp), ALLOCATABLE            :: aod_c_s(:,:,:,:), aod_f_s(:,:,:,:), &
                                      ssa_c_s(:,:,:,:), ssa_f_s(:,:,:,:), &
                                      asy_c_s(:,:,:,:), asy_f_s(:,:,:,:), &
                                      aod_c_f(:,:,:,:),                   &
                                      ssa_c_f(:,:,:,:),                   &
                                      ...
                                      z_km_aer_c_mo(:,:,:,:), &
                                      z_km_aer_f_mo(:,:,:,:)
!  REAL(dp), ALLOCATABLE            :: discf(:,:), aero_strat(:,:)
  INTEGER, PARAMETER               :: lev_clim=40
  REAL(dp)                         :: dz_clim, rdz_clim
...
! allocate memory for optical properties
  ...
  ALLOCATE(aod_c_f(ldc%nproma,jpband,ldc%ngpblks,ntime))
  ALLOCATE(ssa_c_f(ldc%nproma,jpband,ldc%ngpblks,ntime))
  ALLOCATE(asy_c_f(ldc%nproma,jpband,ldc%ngpblks,ntime))
...
! !INPUT PARAMETERS
  ...
  REAL(dp),INTENT(in) :: ppd_hl(kbdim,klev)  ,& ! layer pressure thickness 
...
! !OUTPUT PARAMETERS
  REAL(dp),INTENT(out),DIMENSION(kbdim,klev,nb_lw):: &
   paer_tau_lw_vr      !aerosol optical depth (far IR)
  REAL(dp),INTENT(out),DIMENSION(kbdim,klev,nb_sw):: &
...
! !LOCAL VARIABLES
  ...
  INTEGER                     :: jl,jk,jwl
  REAL(dp), DIMENSION(kbdim,klev)   :: zh, &    ! altitude above surface
                                       zdeltag, & ! layer thickness
                                       zh_vr, &
                                       zdeltag_vr
  REAL(dp), DIMENSION(kbdim)        :: zq_int ! integral height profile
  REAL(dp), DIMENSION(kbdim,nb_lw)  :: zs_i
  REAL(dp), DIMENSION(kbdim,nb_sw)  :: zt_c, zt_f, &
                                       zs_c, zs_f, &
  ...
  REAL(dp), DIMENSION(kbdim,klev)   :: zq_aod_c, zq_aod_f ! altitude profile
                                       ! on echam grid (coarse and fine mode)
  INTEGER, DIMENSION(kbdim)         :: kindex ! index field
  REAL(dp), PARAMETER               :: rdog=rd/g
...
!     echam for altitude profiles
     ...
     zh(1:kproma,klev)=0.5_dp*ppd_hl(1:kproma,klev)*tk_fl(1:kproma,klev)* &
                       rdog/pp_fl(1:kproma,klev)+geosp(1:kproma,krow,1)*qg
     DO jk=klev-1,1,-1
       zh(1:kproma,jk)=zh(1:kproma,jk+1) + &
                       0.5_dp*ppd_hl(1:kproma,jk+1)*tk_fl(1:kproma,jk+1)* &
                       rdog/pp_fl(1:kproma,jk+1) + &
                       0.5_dp*ppd_hl(1:kproma,jk)*tk_fl(1:kproma,jk)* &
...
!!$     DO jk=1,klev
!!$        zh(1:kproma,jk)=(pgeom1(1:kproma,jk)+geosp(1:kproma,krow))*qg
!!$     END DO
     DO jk=1,klev
...
! (ii) calculate height profiles on echam grid for coarse and fine mode
     zq_aod_f(1:kproma,1:klev)=0._dp
     zq_aod_c(1:kproma,1:klev)=0._dp
     DO jk=1,klev
        kindex(1:kproma)=MAX(INT(zh_vr(1:kproma,jk)*rdz_clim+0.5_dp),1)
        DO jl=1,kproma
...
! (iii) far infrared
     zs_i(1:kproma,1:nb_lw)=1._dp-(wgt1_m*ssa_c_f(1:kproma,1:nb_lw,krow,nmw1_m)+ &
...
! aerosol optical depth 
     ...
     WHERE (paer_tau_sw_vr(1:kproma,jk,1:nb_sw) /= 0._dp) 
        paer_cg_sw_vr(1:kproma,jk,1:nb_sw)=paer_cg_sw_vr(1:kproma,jk,1:nb_sw)/&
                                          ...
                                          paer_tau_sw_vr(1:kproma,jk,1:nb_sw)
     ELSEWHERE
        paer_cg_sw_vr(1:kproma,jk,1:nb_sw)=0._dp
     END WHERE
  ENDDO
...
! !INPUT PARAMETERS
  ...
  REAL(dp), INTENT(out)          :: aod(kbdim,nwl,ngpblks,imnthb:imnthe), &
                                    ssa(kbdim,nwl,ngpblks,imnthb:imnthe), &
                                    asy(kbdim,nwl,ngpblks,imnthb:imnthe)
  REAL(dp), INTENT(out)          :: aer_ex(kbdim,lev_clim,ngpblks,imnthb:imnthe)
  REAL(dp), INTENT(out), OPTIONAL:: asl(kbdim,ngpblks)
  REAL(dp), INTENT(out)          :: dz_clim

! !LOCAL VARIABLES
  REAL(dp), POINTER              :: faero1(:,:,:),faero2(:,:,:), &
                                    faero3(:,:,:)
  REAL(dp), POINTER              :: field_g3(:,:,:)
  REAL(dp), POINTER              :: field_g2(:,:)
  LOGICAL                        :: lex
