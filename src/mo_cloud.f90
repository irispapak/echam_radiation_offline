...
!----------------------------------
  ...
  REAL(dp)     :: tbetai(0:1,0:nbetaq,0:nbetax+1) ! betai table for q=cbeta_pq
  REAL(dp)     :: tbetai0(0:nbetaq,0:nbetax+1) ! betai table for q=cbeta_pq
  REAL(dp)     :: tbetai1(0:nbetaq,0:nbetax+1) ! betai table for q=cbeta_pq
...
!      CALL finish ('mo_cloud', 'Truncation not supported.')
    ENDIF
! 
!                199 Level, middle atmosphere
!
  ELSE IF(nlev == 199  .AND. lmidatm) THEN
    IF (nn == 255) THEN
      cauloc  = 5.0_dp
!    ELSE
!      CALL finish ('mo_cloud', 'Truncation not supported.')
...
!
  ...
  END DO
  fjbmax = REAL(jbmax,dp)
!
  jbmin1  = jbmin-1
  fjbmin1 = REAL(jbmin1,dp)
!
