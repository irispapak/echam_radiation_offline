...
CONTAINS
  ...
  SUBROUTINE WMO_tropopause(kproma, kbdim, klev, &
    ...
    !     interface
    !     ---------
    !     call WMO_tropopause(kproma, kbdim, klev, krow, ptm1, papm1, ptropo)
    !
    ...
    !
    !
    USE mo_constants,    ONLY: rd, cpd, g
    USE mo_cloud,        ONLY: ncctop, nccbot
!LT
!    USE mo_time_control, ONLY: lresume
    ...
    ! array arguments
    REAL(dp), INTENT(in)  :: ptm1(kbdim,klev), papm1(kbdim,klev)
    REAL(dp), INTENT(inout) :: ptropo(kbdim)

    INTEGER, INTENT(out) :: ktrpwmo(kbdim), ktrpwmop1(kbdim)

    ! local arrays
#ifdef VECTOR
    REAL(dp) :: ztropov(kproma)
#else
    REAL(dp) :: ztropo(kproma)
#endif

    REAL(dp) :: zpmk(kproma,klev), zpm(kproma,klev),   za(kproma,klev)
    REAL(dp) :: zb(kproma,klev),   ztm(kproma,klev),   zdtdz(kproma,klev)
    REAL(dp) :: zplimb(kproma), zplimt(kproma)

    REAL(dp) :: zptph, zp2km, zkappa, zkapm1, zzkap, zfaktor, zgwmo, zdeltaz
    REAL(dp) :: zag, zbg, zdp
...
!$OMP THREADPRIVATE (linit)
    ...
    REAL(dp) :: zptphv(kproma), zp2kmv(kproma), zasumv(kproma)
    REAL(dp) :: zptph_temp(kproma), zp2km_temp(kproma), zasum_temp(kproma)
#else
    INTEGER :: kcount

    REAL(dp) :: zasum, zamean
#endif
    REAL(dp) :: zpapm1(kproma,klev)
!LT
!    IF (.NOT. lresume) THEN
      IF (.NOT. linit) THEN
        ptropo (1:kproma) = 20000.0_dp
        linit = .TRUE.
      END IF
!    END IF
    
    zkappa   = rd/cpd                   ! 0.286
    zkapm1   = zkappa-1.0_dp
    zzkap    = 1.0_dp/zkappa
    zfaktor  = (-1.0_dp)*g/rd           ! -9.81/287.0
    zgwmo    = -0.002_dp
    zdeltaz  = 2000.0_dp
...
!LK may generate problem on NEC !cdir collapse
    DO jk = iplimt-1, iplimb+1
      DO jl = 1, kproma
        ...
        zpmk(jl,jk) = 0.5_dp*(zpapm1(jl,jk-1)+zpapm1(jl,jk))
      ...
      DO WHILE (n_still_still > 0 .AND.  jj >= iplimt-1)
        DO jlv = 1, n_still_still
          ...
          IF (zpm(jl,jj) <= zptph .AND.zpm(jl,jj) >= zp2km )THEN
            ...
            IF( zasumv(jlv) <= zgwmo* REAL(kcountv(jlv),dp) ) THEN
              n_still_new = n_still_new + 1
    ...
    nproma_loop: DO jl = 1, kproma
      vertical_loop: DO jk = iplimb+1, iplimt-1, -1
        ...
        IF (zdtdz(jl,jk) >  zgwmo .AND.  &     ! dt/dz > -2K/km
          ...
          kcount  = 0.0_dp                                        ! number of levels above
          ...
          IF (zptph >= zplimt(jl)) THEN
            vertical_sub_loop: DO jj = jk, iplimt-1, -1
              ...
              IF (zpm(jl,jj) <= zptph .AND. zpm(jl,jj) >= zp2km )THEN
                ...
                kcount= kcount+1
                zamean = zasum/REAL(kcount,dp)            ! dt/dz mean
...
#ifdef VECTOR
    WHERE (ztropov(1:kproma) > 0.0_dp)
      ptropo(1:kproma) = ztropov(1:kproma)
   END WHERE
#else
    WHERE (ztropo(1:kproma) > 0.0_dp)
...
!LK    ENDDO
    ...
    DO jk = iplimt-1, iplimb+1
      DO jl = 1, kproma
        ...
        zdp = 0.5_dp*(papm1(jl,jk+1)-papm1(jl,jk))
