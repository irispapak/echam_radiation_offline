...
MODULE mo_lrtm
  ...
  USE mo_constants,    ONLY: grav=>g, amd, amw
  USE parrrtm,         ONLY: nbndlw, nmol, ngptlw, ngb
...
CONTAINS
  ...
  SUBROUTINE lrtm     (                            &
    ...
    INTEGER(widx) :: jl, jk, ib, ig ! loop indicies
    ...
    ! 
    CALL lrtm_rtrnmr(                                                &
         & INT(kproma,widx),           INT(kbdim,widx),              &
         & INT(klev,widx),             INT(istart,widx),             &
         & INT(iend,widx),             iout,                         &
...
#ifdef CHK_FLX
        PRINT '(A10,I4,I4,4E24.15)','lrtm_flx',jl,jk,uflx(jl,jk+1),dflx(jl,jk+1),uflxc(jl,jk+1),dflxc(jl,jk+1)
#endif
      ENDDO
    ENDDO
  ...
  SUBROUTINE inatm (                                 &
    ...
    DO jl = 1, kproma  ! loop over columns
       ...
       DO jk = 1, klev
          summol = 0.0_wp
          DO imol = 2, nmol
             summol = summol + wkl_2d(jl,imol,jk)
          ENDDO
          wbrodl(jl,jk) = coldry_2d(jl,jk) - summol
          amttl = amttl + coldry_2d(jl,jk)+wkl_2d(jl,1,jk)
          wvttl = wvttl + wkl_2d(jl,1,jk)
       ENDDO
       wvsh = (amw * wvttl) / (amd * amttl)
       pwvcm(jl) = wvsh * (1.e3_wp * psfc(jl)) / (1.e2_wp * grav)
    ENDDO
